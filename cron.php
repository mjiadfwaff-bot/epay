...
elseif($_GET['do']=='notify'){
	$limit = 20; //每次重试的订单数量
	for($i=0;$i<$limit;$i++){
		$srow=$DB->getRow("SELECT * FROM pre_order WHERE endtime>=DATE_SUB(NOW(), INTERVAL 1 DAY) AND notify>0 AND notifytime IS NOT NULL AND notifytime<NOW() ORDER BY notifytime ASC LIMIT 1");
		if(!$srow)break;
		...
		if(do_notify($url['notify'])){
			...
			if($conf['auto_check_notify'] == 1){
				...
				if($count > 0){
						...
						if($userrow['pay'] == 1){
							$orders = $DB->getAll("SELECT trade_no,notify FROM pre_order WHERE uid='{$srow['uid']}' and status>0 order by trade_no desc limit {$count}");
							$failcount = 0;
...
elseif($_GET['do']=='notify2'){
	$limit = 20; //每次重试的订单数量
	for($i=0;$i<$limit;$i++){
		$srow=$DB->getRow("SELECT * FROM pre_order WHERE endtime>=DATE_SUB(NOW(), INTERVAL 1 DAY) AND notify=-1 ORDER BY endtime DESC LIMIT 1");
		if(!$srow)break;

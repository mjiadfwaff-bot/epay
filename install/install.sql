...
CREATE TABLE `pre_order` (
 ...
 KEY `notify_notifytime_endtime` (`notify`,`notifytime`,`endtime`),
 KEY `uid_status_date` (`uid`,`status`,`date`),
 KEY `channel_status_date` (`channel`,`status`,`date`),
 KEY `buyer_date_status` (`buyer`,`date`,`status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

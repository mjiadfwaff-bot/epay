...
ALTER TABLE `pre_order`
ADD INDEX `status_addtime` (`status`,`addtime`),
ADD INDEX `notify_notifytime_endtime` (`notify`,`notifytime`,`endtime`),
ADD INDEX `uid_status_date` (`uid`,`status`,`date`),
ADD INDEX `channel_status_date` (`channel`,`status`,`date`),
ADD INDEX `buyer_date_status` (`buyer`,`date`,`status`);
